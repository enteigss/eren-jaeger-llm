...
class TranscriptToJsonConverter:
    def __init__(self, output_dir: str) -> Transcript:
    ...
    def convert(self, file_path: str) -> Transcript:
            """Parse a transcript file and return a Transcript object."""
            with open(file_path, 'r', encoding='utf-8') as f:
                lines = f.readlines()

            # Initialize the JSON structure
            filename = os.path.basename(file_path)
            title = self.clean_title(os.path.splitext(filename)[0])
            ...
            scenes = []
            for scene_data in transcript["scenes"]:
                events = []
                for event in scene_data["dialogue"]:
            ...
            return Transcript(
                ...
                episode_info=transcript["episode_info"],
                scenes=scenes
            )

    def save_jsons(self):
        """Save all transcripts in the raw directory to JSON format."""
        project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

        transcript_dir = os.path.join(project_root, "tests", "transcript-txt")
        output_dir = os.path.join(project_root, "tests", "transcript-json")
        ...
        for filename in os.listdir(transcript_dir):
            if filename.endswith('.txt'):
                ...
                transcript = self.convert(file_path)
                transcript = asdict(transcript)
            
                # Create the JSON filename
                json_filename = self.clean_title(os.path.splitext(filename)[0]) + '.json'
